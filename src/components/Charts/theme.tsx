...
export function ChartTooltip({ active, payload, label, valueFormatter }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-overlay border border-border bg-card px-3 py-2 text-caption shadow-md">
      ...
      <div className="flex flex-col gap-1">
        {payload.map((p, i) => (
          <div key={i} className="flex items-center gap-2 text-ink-secondary">
            <span className="size-2 shrink-0 rounded-full" style={{ background: p.color ?? p.fill }} aria-hidden />
            <span>{p.name}</span>
